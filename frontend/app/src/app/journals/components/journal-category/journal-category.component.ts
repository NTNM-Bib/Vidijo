...
export class JournalCategoryComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    const color1: string = Color(this.category.color).darken(0.1).toString();
    const color2: string = Color(this.category.color).darken(0.2).toString();
    const color3: string = Color(this.category.color).darken(0.3).toString();
  ...
  get isSpecialCategory(): boolean {
    return (
      this.category._id === "all" ||
      this.category._id === "favorites" ||
      this.category._id === ""
    );
  }
}
