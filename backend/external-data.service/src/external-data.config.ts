...
export class ExternalDataConfigClass extends DefaultConfigClass {
    ...
    private getLocalCoverFolder(): string {
        if (!process.env.LOCAL_COVER_FOLDER) {
            const localCoverFolderMissingError: Error = new Error("A destination folder for saving journal covers is not set in your .env file. Please set it with 'LOCAL_COVER_FOLDER=<your local cover folder>'");
    ...
    private getPublicCoverFolderUrl(): string {
        if (!process.env.PUBLIC_COVER_URL) {
            const publicCoverFolderUrlMissingError: Error = new Error("Please provide the URL to the public cover folder in the .env file. You can set it with 'PUBLIC_COVER_URL=<your public cover folder url>'");
