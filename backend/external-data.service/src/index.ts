...
App.listen(ExternalDataConfig.PORT, async () => {
  ...
  Logger.log(`Starting server on port ${ExternalDataConfig.PORT.toString()}...`);
...
Mongoose.connection.on("disconnected", () => {
  const disconnectedError: Error = new Error(`Disconnected from database ${ExternalDataConfig.MONGODB_URI}`);
