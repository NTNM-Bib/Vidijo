...
export class ApiConfigClass extends DefaultConfigClass {
    ...
    private getSessionSecret(): string {
        if (!process.env.SESSION_SECRET) {
            const sessionSecretMissingError: Error = new Error("SESSION_SECRET is missing. Set this value to a long random string in the .env file (recommended length of 96 characters)");
    ...
    private getMailHost(): string {
        if (!process.env.MAIL_HOST) {
            const mailHostMissingError: Error = new Error("MAIL_HOST is missing. Please provide the host address of your mail server for sending mails to users in the .env file (MAIL_HOST=<your mail host>)");
    ...
    private getMailPort(): number {
        if (!process.env.MAIL_PORT) {
            console.warn("MAIL_PORT not set in .env file. Using default SMTPS port 465 instead. If your want to change the port, add the following to the .env file: MAIL_PORT=<your port>");
        }
    ...
    private getMailUsername(): string {
        if (!process.env.MAIL_USERNAME) {
            const mailUsernameMissingError: Error = new Error("MAIL_USERNAME is missing. Please provide the username of the mail account for sending mails to users in the .env file (MAIL_USERNAME=<your username>)");
    ...
    private getMailPassword(): string {
        if (!process.env.MAIL_PASSWORD) {
            const mailPasswordMissingError: Error = new Error("MAIL_PASSWORD is missing. Please provide the password of the mail account for sending mails to users in the .env file (MAIL_PASSWORD=<your password>)");
    ...
    private getInstitutionName(): string {
        if (!process.env.INSTITUTION_NAME) {
            const institutionNameMissingError: Error = new Error("The name of your institution is not set in your .env file. Please set it with 'INSTITUTION_NAME=<your institution name>'");
    ...
    private getInstitutionLogoUri(): string {
        if (!process.env.INSTITUTION_LOGO_URI) {
            const institutionLogoUriMissingError: Error = new Error("Please provide the URI to the institution logo in the .env file. You can set it with 'INSTITUTION_LOGO_URI=<your institution logo URI>'");
    ...
    private getApiUriHosted(): string {
        if (!process.env.API_URI_HOSTED) {
            const apiUriHostedMissingError: Error = new Error("Please provide the URI to the hosted Vidijo-API in the .env file. You can set it with 'API_URI_HOSTED=<URI of hosted API>'");
