...
function updateArticlesLoop() {
  Updater.updateOldestJournal()
    .then(() => {
      return setTimeout(updateArticlesLoop, UpdaterConfig.UPDATE_INTERVAL * 60 * 1000); // convert to ms
    })
    .catch((err: Error) => {
      Logger.error(err);
...
Mongoose.connection.on("disconnected", () => {
  const disconnectedError: Error = new Error(`Disconnected from database ${UpdaterConfig.MONGODB_URI}`);
...
process.on('unhandledRejection', (reason, promise) => {
  Logger.error(new Error(`Unhandled promise rejection at: promise ${JSON.stringify(promise)}, reason: ${reason}.`));
...
connectToDatabase()
  .then((databaseUrl: string) => {
    ...
    Logger.log(`Updating articles every ${UpdaterConfig.UPDATE_INTERVAL} minutes`);
    updateArticlesLoop();
  })
  .catch((err: Error) => {
    Logger.error(err);
